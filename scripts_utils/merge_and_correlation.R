...
for (j in paramspace_cfDNA$cfDNA_folder){
        tumor <- read.table(paste("data/", pt, "/", j, "/de_novo_germline_reference/intersection_unique_tumor_cfDNAintersection.txt", sep=""))
        colnames(tumor) <- c("kmer", "tumor")
        cfDNA <- read.table(paste("data/", pt, "/", j, "/de_novo_germline_reference/cfDNA_kmers_unique_tumor_kmers_intersect.txt", sep=""))
