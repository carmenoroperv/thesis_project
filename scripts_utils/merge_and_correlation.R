...
for (j in paramspace_cfDNA$cfDNA_folder){
        tumor <- read.table(paste("data/", pt, "/", j, "/all_germline_filtered_bams_tumor_ci5_cs1e9/intersection_unique_tumor_cfDNAintersection.txt", sep=""))
        colnames(tumor) <- c("kmer", "tumor")
        cfDNA <- read.table(paste("data/", pt, "/", j, "/all_germline_filtered_bams_tumor_ci5_cs1e9/cfDNA_kmers_unique_tumor_kmers_intersect.txt", sep=""))
